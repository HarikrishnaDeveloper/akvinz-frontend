...
export default function BankDetailsFormPage() {
  ...
  const fetchCustomerDetails = async (fullMobileNumber: string) => {
    try {
      ...
      if (data.success) {
        ...
        if (data.customer.bankAccountHolderName) setBankAccountHolderName(data.customer.bankAccountHolderName);
        if (data.customer.bankName) setBankName(data.customer.bankName);
        if (data.customer.bankIfscCode) setBankIfscCode(data.customer.bankIfscCode);
        if (data.customer.bankAccountNumber) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitError("");
    try {
      const res = await fetch(`${API_URL}/account/bank-details`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (data.success) {
        ...
        setSubmitError(data.message || "Failed to save bank details");
      }
    } catch (err) {
      ...
      <main className="flex-grow max-w-3xl mx-auto w-full px-4 py-10 relative">
        {step < 3 && (
          <div className="text-center mb-10">
            <h1 className="text-3xl sm:text-4xl font-bold text-white mb-4">
              {step === 1 ? "Customer Verification" : "Bank Account Details"}
            </h1>
            <p className="text-gray-400 text-sm sm:text-base max-w-md mx-auto leading-relaxed">
              {step === 1
                ? "Enter your registered mobile number to proceed."
                : "Provide the bank account you'd like us to use for any future payments to you."}
            </p>
          </div>
        )}
        ...
        {step === 2 && customer && (
          <div className="bg-[#1a1f30]/80 border border-gray-700/50 rounded-2xl p-6 sm:p-8 backdrop-blur-sm space-y-8 max-w-xl mx-auto">
            ...
            <div className="bg-[#131724] p-5 rounded-xl border border-gray-700">
              <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-1">Bank Account Details</h3>
              <p className="text-xs text-gray-500 mb-4">
                This account will be used for any future payments we need to send you.
              </p>
            ...
            <div className="pt-2">
              <button
                ...
                disabled={isSubmitting || !bankDetailsFilled || !accountNumbersMatch || ifscStatus === "invalid"}
        ...
        {step === 3 && (
          <div className="bg-[#1a1f30]/80 border border-gray-700/50 rounded-2xl p-8 sm:p-12 backdrop-blur-sm text-center max-w-xl mx-auto">
            ...
            <h2 className="text-3xl font-bold text-white mb-4">Bank Details Saved</h2>
